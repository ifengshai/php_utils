...
class RequestLogProcessor extends Processor
{
    ...
    {
        $enabled = config('fslog.request_log.enabled', false);
        if (!$enabled) {
            return;
        }
    ...
    public function getPath()
    {
        ...
        $this->path = config('fslog.request_log.path');
        return $this->path;
    }
    ...
    public function getType()
    {
        ...
        $this->type = config('fslog.request_log.type');
        return $this->type;
    }
    ...
    public function getFormat()
    {
        if (is_null($this->format)) {
            $this->format = config('fslog.request_log.format', self::DEFAULT_EXT_FORMAT);
        }
        return $this->format;
    }
    ...
    protected function initQueryListen()
    {
        ...
        try {
            $events->listen($listener, function ($query, $bindings = null, $time = null, $connectionName = null) use ($queryCollector) {
            ...
            throw new \Exception(
                'FsLog DB listen failed: ' . $e->getMessage(),
                $e->getCode(),
                $e
            );
        }
    ...
    public function addQuery($query = '', $bindings = null, $time = null, $connectionName = null)
    {
        if (!is_null($this->callback)) {
            call_user_func_array($this->callback, [$query, $bindings, $time, $connectionName]);
        }
        ++$this->dbCount;
        $this->dbTime += $time;
    }
