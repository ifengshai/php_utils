...
namespace Fengsha\Utils\Log\Handlers;

use Psr\Log\LoggerInterface as PsrLoggerInterface;
use Monolog\Logger as Monolog;
use Fengsha\Utils\Log\Extend\LineFormatter;
use Fengsha\Utils\Log\Extend\FluentdFormatter;
use Monolog\Handler\RotatingFileHandler;
use Monolog\Handler\StreamHandler;
use Fluent\Logger\FluentLogger;
use Fengsha\Utils\Log\Extend\FluentdHandler;
use Fengsha\Utils\Log\Exceptions\FsLogUnexpectedValueException;
use Fengsha\Utils\Log\Exceptions\FsLogInvalidArgumentException;
...
class MonologHandler extends Handler implements PsrLoggerInterface
{
    ...
    {
        if (strlen($path) === 0) {
            throw new FsLogInvalidArgumentException("Path is empty");
        }
        $this->path = $path;
    }
    ...
    public function setType($type)
    {
        if (!in_array($type, $this->typeList)) {
            throw new FsLogUnexpectedValueException("Unsupported type [{$type}]");
        }
        $this->type = $type;
    }
     ...
     * @return \Fengsha\Utils\Log\Extend\FluentdFormatter
     */
