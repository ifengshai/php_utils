...
class RequestProcessor
{
    ...
    protected function getRequestId()
    {
        if (is_null(self::$requestId)) {
            $requestIdConf = config('fslog.trace_id');
    ...
    {
        if (is_null(self::$rpcId)) {
            self::$rpcId =  empty($_SERVER['HTTP_X_REQUEST_RPCID']) ? '0' : $_SERVER['HTTP_X_REQUEST_RPCID'];
        }
        return self::$rpcId;
    }
    ...
    protected function getUserId()
    {
        if (empty(self::$userId)) {
            self::$userId =  empty($_SERVER['HTTP_X_USERID']) ? '0' : $_SERVER['HTTP_X_USERID'];
        }
        return self::$userId;
    }
